...
### Streamlit 여신 리스크 조회 도구

신용등급과 대출 목적을 선택하면 해당 세그먼트의 과거 부실률과
등급 평균 대비 수준을 조회할 수 있는 웹 도구를 제작함.

![Streamlit App](images/streamlit_app.png)

**구성**
- 입력: 신용등급(A~G), 대출 목적(12종)
- 출력: 부실률, 등급 평균 대비 배수, 표본 수, 평균 DTI·대출금액
- 판정: n8n 알림 시스템과 동일한 기준(1.5배 초과 주의 / 1.2배 초과 관찰) 적용
- 표본 300건 미만 조합은 해석 주의 안내를 함께 표시

**설계 의도**
- 대시보드가 "전체 현황을 보는" 용도라면, 이 도구는 "개별 심사 건을 조회하는"
용도로 설계함. 심사역이 신청 건을 받았을 때 해당 프로필의 과거 실적을
즉시 확인하는 상황을 가정
- 알림 워크플로(n8n)와 조회 도구(Streamlit)가 동일한 `ratio` 기준을 공유하도록 하여,
모니터링과 심사에 일관된 리스크 정책이 적용되도록 구성

**분석 결과**
- 등급 평균 대비 1.5배를 초과하는 조합은 88개 중 4개
(A/renewable_energy 2.04배, A/small_business 1.84배, B/educational 2.36배,
C/educational 1.52배)
- 모두 용도 특성상 리스크가 높은 목적이며, 건수 비중이 큰 주류 목적
(debt_consolidation, credit_card)은 포함되지 않음
- 즉 "등급은 양호하나 목적이 특이한" 조합이 실제 위험 구간이며,
등급 단일 기준 심사에서 놓치기 쉬운 지점임을 확인

**참고**
- 표본 100건 이상인 조합만 조회 대상에 포함(`HAVING COUNT(*) >= 100`)
- DTI를 입력 축으로 추가하는 방안도 검토했으나, 조합 수가 3배 이상 증가해
조합당 표본이 과도하게 분산되는 문제가 있어 제외함